...
def test_pure_rolling_gives_zero_forces():
    ...
    wheel = Wheel()
    v_x = 20.0
    y = wheel._func_alg(np.array([0.0, v_x / wheel.R_w, v_x, 0.0, 0.0]))
...
def test_standstill_is_well_defined():
    ...
    wheel = Wheel()
    assert np.allclose(wheel._func_alg(np.zeros(5)), 0.0)


def test_algebraic_chain_matches_equations():
    """_func_alg reproduces the documented kinematics -> tire -> rotation chain."""
    wheel = Wheel(l=1.2, s=0.0, R_w=0.30, F_z=4000.0, v_eps=1.0)
    delta, omega, v_x, v_y, r = 0.08, 40.0, 15.0, 0.4, 0.25
    F_x, F_y, M_y = wheel._func_alg(np.array([delta, omega, v_x, v_y, r]))

    v_cx = v_x - wheel.s * r
    v_cy = v_y + wheel.l * r
    kappa = (wheel.R_w * omega - v_cx) / np.sqrt(v_cx**2 + wheel.v_eps**2)
    alpha = delta - np.arctan2(v_cy, v_cx)
    F_x_t, F_y_t = wheel.tire.forces(kappa, alpha, wheel.F_z)
...
def test_braking_slip_gives_negative_force_and_torque():
    ...
    wheel = Wheel()
    v_x = 20.0
    F_x, F_y, M_y = wheel._func_alg(np.array([0.0, 0.5 * v_x / wheel.R_w, v_x, 0.0, 0.0]))
    assert F_x < 0.0
...
def test_tire_model_is_swappable_and_pacejka_saturates():
    ...
    u = np.array([0.0, 0.0, 20.0, 0.0, 0.0])
    F_z = 4000.0
    linear = Wheel(F_z=F_z, tire=LinearTire())
    pacejka = Wheel(F_z=F_z, tire=SimplePacejka())
...
def _closed_loop_sim(delta, omega, v_x0, duration, tire=None):
    ...
    front wheel steered, chassis velocities fed back to both wheels."""
    def make_tire():
        return None if tire is None else tire()

    wf = Wheel(l=1.2, tire=make_tire())
    wr = Wheel(l=-1.4, tire=make_tire())
...
def test_closed_loop_pacejka_understeers_more_than_linear():
    """Near zero steer SimplePacejka agrees with a LinearTire built from the
    bridge stiffnesses C = B*C*mu*F_z; at high lateral acceleration the
    saturating tire yields less yaw rate than the unbounded linear one."""
    F_z = 4000.0
    pac = SimplePacejka()
    bridged = lambda: LinearTire(C_kappa=pac.B_x * pac.C_x * pac.mu_x * F_z,
                                 C_alpha=pac.B_y * pac.C_y * pac.mu_y * F_z)
    kwargs = dict(omega=20.0 / 0.30, v_x0=20.0, duration=15.0)

    delta = 0.01                          # linear range: rungs agree
    _, d_lin = _closed_loop_sim(delta=delta, tire=bridged, **kwargs)
    _, d_pac = _closed_loop_sim(delta=delta, tire=SimplePacejka, **kwargs)
    assert np.isclose(d_pac[2][-1], d_lin[2][-1], rtol=0.02)

    delta = 0.08                          # a_y ~ several m/s^2: saturation bites
    _, d_lin = _closed_loop_sim(delta=delta, tire=bridged, **kwargs)
    _, d_pac = _closed_loop_sim(delta=delta, tire=SimplePacejka, **kwargs)
    assert d_pac[2][-1] < d_lin[2][-1]
